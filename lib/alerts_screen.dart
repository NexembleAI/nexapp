...
class _AlertsScreenState extends State<AlertsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final needs = (_alerts ?? []).where((a) => a.needsAction).toList()
      ..sort((a, b) => b.createdAt.compareTo(a.createdAt));
    // Handled-but-unresolved alerts; resolved ones leave the inbox.
    final earlier = (_alerts ?? [])
        .where((a) =>
            a.status == AlertStatus.ack || a.status == AlertStatus.snoozed)
        .toList()
      ..sort((a, b) => b.createdAt.compareTo(a.createdAt));
    ...
    switch (alert.reason) {
        ...
        return Row(
          children: [
            ...
            Expanded(
              child: Text(
                l.reasonNoVisit(
                    alert.daysSinceVisit ?? 0, alert.thresholdDays ?? 0),
      ...
      case AlertReason.nextActivityOverdue:
        return Text(
          ...
          style: theme.textTheme.bodySmall
              ?.copyWith(color: AppTheme.mutedLabel(theme.brightness)),
        );
...
class _EarlierCard extends StatelessWidget {
  ...
  const _EarlierCard({required this.alert});
